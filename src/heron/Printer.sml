...
fun string_of_expr e = case e of
  ...
  | SporadicOn_Abs (cmeas, ctick)                         => (string_of_clk ctick) ^ " sporadic\226\153\175 ? on " ^ (string_of_clk cmeas)
...
(*| Implies (master, slave)                                 => (string_of_clk master) ^ " implies " ^ (string_of_clk slave) *)
  ...
  | TimeDelayedBy_Abs (master, measuring, NONE, slave)             => (string_of_clk master) ^ " time delayed\226\153\175 by ? on " ^ (string_of_clk measuring) ^ " implies " ^ (string_of_clk slave)
  | TimeDelayedBy_Abs (master, measuring, SOME (reset), slave)     => (string_of_clk master) ^ " time delayed\226\153\175 by ? on " ^ (string_of_clk measuring) ^ " with reset on " ^ (string_of_clk reset) ^ " implies " ^ (string_of_clk slave)
