...
"csv"				 => (Tokens.CSV(!pos,!pos));
"@select"                    => (Tokens.DIR_SELECT(!pos,!pos));
"select"                     => (Tokens.SELECT(!pos,!pos));
"in"                         => (Tokens.IN(!pos,!pos));
"#"                          => (Tokens.SHARP(!pos,!pos));
"@ctl"                       => (Tokens.DIR_CTL_FORMULA(!pos,!pos));
"0x"({alpha}|{digit})+       => (Tokens.NUM (valOf (StringCvt.scanString (Int.scan StringCvt.HEX) yytext), !pos,!pos));
"/\\"                        => (Tokens.CONJ_CLAUSE(!pos,!pos));
"\\/"                        => (Tokens.DISJ_CLAUSE(!pos,!pos));
"!"                          => (Tokens.CTL_NOT(!pos,!pos));
"=>"                         => (Tokens.CTL_IMPLIES(!pos,!pos));
"<=>"                        => (Tokens.CTL_IFF(!pos,!pos));
"AX"                         => (Tokens.CTL_AX(!pos,!pos));
"EX"                         => (Tokens.CTL_EX(!pos,!pos));
"AF"                         => (Tokens.CTL_AF(!pos,!pos));
"EF"                         => (Tokens.CTL_EF(!pos,!pos));
"AG"                         => (Tokens.CTL_AG(!pos,!pos));
"EG"                         => (Tokens.CTL_EG(!pos,!pos));
"A["                         => (Tokens.CTL_A_LBRACK(!pos,!pos));
"E["                         => (Tokens.CTL_E_LBRACK(!pos,!pos));
"U"                          => (Tokens.CTL_U(!pos,!pos));
